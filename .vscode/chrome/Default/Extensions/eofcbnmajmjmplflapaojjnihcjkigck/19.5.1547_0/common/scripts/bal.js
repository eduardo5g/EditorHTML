...
(function(_, EventEmitter) {
    ...
    AvastWRC.bal = {
        ...
        init: function() {
            ...
            sing = this;
            var defSettings = AvastWRC.storageCache.get("settings") || CORE_DEFAULT_SETTINGS;

            if (!defSettings.userSPPoppupSettings.privacy) {
                defSettings.userSPPoppupSettings.privacy = CORE_DEFAULT_SETTINGS.userSPPoppupSettings.privacy;
                if (AvastWRC.Utils.getBrowserInfo().isFirefox() && AvastWRC.CONFIG.NEW_INSTALL) defSettings.userSPPoppupSettings.privacy.accepted = !!AvastWRC.CONFIG.OPT_IN;
            }
        ...
        bootstrapInit : function (edition) {
            var features = CORE_DEFAULT_SETTINGS.features;
        ...
        getLandingPageURL: function () {
            ...
            let url = {
                ...
                variant1: `https://platform.${brandingSpecifics.brandingType}.com/SP/Onboarding/?utm_medium=link&utm_source=safeprice&utm_campaign=safeprice-onboarding&language=${lang}`
            };
...
(function(_,Shepherd) {
    ...
    _.extend(Shepherd,{
        ...
        updateSearchConfig: function (rules) {
            if(!rules){
                return {searchConfigs: {Configs: Shepherd.defaultSearchConfig}};
            } 
            if(!rules.searchConfigs || !rules.searchConfigs.Configs){
                rules.searchConfigs = {Configs: Shepherd.defaultSearchConfig};
                return rules;
            } 
            if(!rules.searchConfigs.Configs.offers){
                rules.searchConfigs.Configs.offers = Shepherd.defaultSearchConfig.offers;
            }
            ...
            if(!rules.searchConfigs.Configs.coupons){
                rules.searchConfigs.Configs.coupons = Shepherd.defaultSearchConfig.coupons;
            }
        ...
        showPanelOnInstallPage: function () {
            if(Shepherd.rules && 
               Shepherd.rules.showPanelOnInstallPage && 
               Shepherd.rules.showPanelOnInstallPage.Configs && 
               Shepherd.isSearchActive()){
                return true;
            }
            else{
                return false;
