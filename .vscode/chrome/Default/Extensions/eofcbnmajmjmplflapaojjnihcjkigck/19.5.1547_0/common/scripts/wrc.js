...
(function (_, PROTO) {
    ...
    _.extend(AvastWRC,
        {
            ...
            init: function () {
                ...
                AvastWRC.CONFIG.OPT_IN = AvastWRC.storageCache.get("OPT_IN");


                // Firefox OPT-IN
                if ((AvastWRC.CONFIG.OPT_IN !== false) && AvastWRC.Utils.getBrowserInfo().isFirefox() && newInstall) {
                    let tabId = null;

                    chrome.tabs.create({url: "opt-in.html"}, function (tab) {
                        tabId = tab.id;
                    });

                    chrome.runtime.onMessage.addListener(function (request, sender, senderResponse) {
                        onOptIn(request, tabId);
                    });
                } else {
                    AvastWRC.bal.init();
                }
            ...
            onSendHeaders: function (details) {
                if (details.type === "main_frame") {
                    ...
                    AvastWRC.bal.sp.setBadge(details.tabId, null, false/*no animation*/);
            ...
            avastConfig: {
                load: function () {
                    ...
                    AvastWRC.CONFIG.PLG_GUID = AvastWRC.CONFIG.PLG_GUID || AvastWRC.Utils.getRandomUID();
    ...
    function onOptIn(request, tabId) {
        ...
        if (request.id === "opt-in-acceptance") {
            AvastWRC.CONFIG.OPT_IN = request.acceptance;
            AvastWRC.storageCache.save("OPT_IN", AvastWRC.CONFIG.OPT_IN);
            if (AvastWRC.CONFIG.OPT_IN) {
                AvastWRC.bal.init();
            } else {
                chrome.tabs.create({url: `https://addons.mozilla.org/firefox/addon/${chrome.runtime.getManifest().name.indexOf("AVG") >= 0 ? "avg" : "avast"}-safeprice/privacy/`}, function (tab) {});
            }
